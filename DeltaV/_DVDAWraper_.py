...
from _DVUANode_ import _DVUANode_
from _DVDAManager_ import _DVDAManager_
from _DVistoryManager_ import _DVistoryManager_
...
class _OPCDAWrapper_:
    ...
    def __init__(self,  name:str= 'OPC.DELTAV.1',nodename:str="PROPLUS", endpoint: str = 'opc.tcp://0.0.0.0:4840'):
        ...
        self.node = _DVUANode_(name=name,nodename=nodename,endpoint=endpoint,application_uri=name)
        self.security = CustomSecurity(wrapper=self)
        self.da_manager = _DVDAManager_(wrapper=self,nodename=nodename) 
        self.user_manager = CustomUserManager()  # 强制初始化，避免 None
        self.server = Server(user_manager=self.user_manager)
        self.history_manager=_DVistoryManager_(wrapper=self)
