...
class CustomDAManager:
        def __init__(self,wrapper,nodename="PROPLUS"):
            ...
            self.opcdeltav = _OPCDA_(node_name=self.nodename,server_name = "OPC.DeltaV.1")
            self.opcda = _OPCDA_(node_name=self.nodename,server_name = "DeltaV.DVSYSsvr.1")
        ...
        def broswer_thread(self):
            ...
            self.wrapper.initialize_com()
            logging.debug(f"CustomDAManager.broswer_thread broswer_thread start")
            try:
                        if not self.opcdeltav.connected:
                            self.opcdeltav.connect()                
                        while not self.wrapper.event.shutdown.is_set():
                            if  self.wrapper.event.broswe_opcda_struture.is_set():                                      
                                    self.structure={}
                                    if self.path !="":
                                        move_to_path = self.opcdeltav.move_to_path(self.opcdeltav.browser, self.path)
                                    else:
                                        move_to_path =True
                                    if move_to_path:  
                                        self.structure = self.opcdeltav.browse_level(
                                        self.opcdeltav.browser, 1, self.max_level, self.path, self.structure
                                        )
                                    logging.debug(f"CustomDAManager.broswer_thread: Browser structure completed: {self.structure}")
                                    self.wrapper.event.update_structure.set()
                                    self.wrapper.event.broswe_opcda_struture.clear()                             
        ...
        def opcda_thread(self):
            ...
            try:
                        ...
                        if not self.opcda.connected:
                            self.opcda.connect()
                        items_to_subscribe=self.items.copy()
                        self.opcda.subscribe(items_to_subscribe, group_name=self.group_name, update_rate=self.update_rate, callback=self.custom_callback)
                        ...
                        while not self.wrapper.event.shutdown.is_set():
                            ...
                            if len(items_to_subscribe) != len(self.items):
                                self.opcda.stop_subscribe(self.group_name)
                                time.sleep(1)
                                items_to_subscribe=self.items.copy()
                                self.opcda.subscribe(items_to_subscribe, group_name=self.group_name, update_rate=self.update_rate, callback=self.custom_callback)
                            ...
                            try:
                                ...
                                count = 0
                                while self.wrapper.event.polling.is_set() and not self.wrapper.event.shutdown.is_set() and (max_count is None or count < max_count) and (max_time is None or time.time() - start_time < max_time):
                                    try:
                                        
                                        results = self.opcda.read(items_to_poll)
                                ...
                                self.wrapper.event.polling.clear()
                            except Empty:
                                pass
                            ...
                            try:
                                ...
                                start_time = time.time()
                                while self.wrapper.event.writing.is_set() and not self.wrapper.event.shutdown.is_set() and (time.time() - start_time < 10):
                                    try:
                                        ...
                                        break
                                if  self.wrapper.event.writing.is_set() and time.time() - start_time >= 10:
                                ...
                                self.wrapper.event.writing .clear()
                            except Empty:
                                pass
                ...
                try:
                    if self.group_name and self.opcda.connected:
                        self.opcda.stop_subscribe(self.group_name)
                        logging.debug(f"CustomDAManager.opcda_thread:Subscription {self.group_name} stopped")
                    if self.opcda.connected:
                        self.opcda.disconnect()
        ...
        async def _wait_for_polling(self):
            while  self.wrapper.event.polling.is_set() and not  self.wrapper.event.shutdown.is_set():
        ...
        async def async_write(self,  values: List[any], items: List[str],group_name: str = "WriteGroup", update_rate: int = 1000):
            ...
            if not  self.wrapper.event.running.is_set():
            ...
            async with self.write_lock:  # 使用锁确保顺序执行
                if  self.wrapper.event.writing.is_set():
            ...
            self.wrapper.event.writing.set()
        ...
        async def create_structure(self,parent_node,structure: Dict = None, base_path: str = None):
            ...
            for key, value in structure.items():
                ...
                if isinstance(value, dict):
                    ...
                    try:
                        # 尝试获取特定子节点
                        await folder.get_child(f"{self.wrapper.node.idx}:{method_name}")
                        method_exists = True
        ...
        async def create_folder(self,base_path: str):
                """Recursively create folder structure in OPC UA based on OPC DA structure."""
                node=self.wrapper.node.da_folder
                
                paths=base_path.split('.')
                
                for i in range(len(paths)):
                    path=".".join(paths[:i+1])
                    if path not in self.wrapper.node.folders:
                        node = await node.add_folder(self.wrapper.node.idx, paths[i])
                        self.wrapper.node.folders[path]=node
                        ...
                        node = self.wrapper.node.folders[path]
        ...
        async def broswe_folder(self,max_level:int=1,base_path: str= ""):
                ...
                async with self.path_lock:  # 使用锁确保顺序执行
                    ...
                    self.wrapper.event.broswe_opcda_struture.set()
                    await asyncio.sleep(1)
                    if not self.wrapper.event.update_structure.is_set():
                    # 等待事件被 clear（需要其他协程调用 .clear()）
                        await self.wrapper.event.update_structure.wait()  # 阻塞直到事件被 clear
                
                    logging.debug(f"CustomDAManager.broswe_folder:structure ={self.structure}") 
                    parent_node = self.wrapper.node.da_folder if base_path == "" else self.wrapper.node.folders.get(base_path)
                    ...
                    self.wrapper.event.update_structure.clear()
                ...
                if data and data[1] != 0:  # 检查数据有效性
                    ...
                    if item not in last_values or last_values[item] != value:
                        ...
                        if item not in self.wrapper.node.nodes:
                            ...
                            # 在目标文件夹下创建节点
                            node_name = item  
                            node = await target_folder.add_variable(self.wrapper.node.idx, node_name, initial_value, varianttype=variant_type)
                            await node.set_writable(True)
                            self.wrapper.node.nodes[item] = node
                            node_id = node.nodeid
                        ...
                        # 更新节点值
                        node = self.wrapper.node.nodes[item]
                        ...
                        try:
                            ...
                            if self.max_updates and self.update_count >= self.max_updates:
                                logging.debug(f"CustomDAManager.add_items: Reached max updates ({self.max_updates}), stopping subscription...")
                                self.wrapper.event.shutdown.set()
                        except ua.UaStatusCodeError as e:
                            logging.error(f"CustomDAManage.add_itemsr:Failed to write {item}: {str(e)}")
                            await self.wrapper.node.last_error_desc.write_value(f"_OPCDAWrapper_.add_items: Failed to write {item}, Error Occured: {str(e)}")
                    else:
                   ...
                   # logging.debug(f"CustomDAManage.update_ua_nodes: cycle update ua nodes ")    
                    for item in self.items:
                        ...
                        try:
                            ...
                            await self.wrapper.node.last_error_desc.write_value(f"CustomDAManage.update_ua_nodes: Failed to write {item}, Error Occured: {str(e)}") 
                    await asyncio.sleep(self.ua_update_rate)  
        ...
        async def update_item(self, parent, path_variant,level_variant):
                    ...
                    if not self.wrapper.user_manager.check_method_permission(50, userrole):
                        ...
                        await self.wrapper.node.last_error_desc.write_value(f"CustomDAManage.update_node_call:Unauthorized attempt to call generate_server_certificate ")
                        raise ua.UaStatusCodeError(ua.StatusCodes.BadUserAccessDenied)
                    try:
                        path = path_variant.Value
                        level = level_variant.Value
                        if not isinstance(path, str):
                                await self.wrapper.node.last_error_desc.write_value("CustomDAManage.update_node_call: Invalid input - path must be a string")
                        ...
                        if any(part == '' or any(char in part for char in special_chars) for part in path_parts) or len(path_parts) < 2:
                            await self.wrapper.node.last_error_desc.write_value("CustomDAManage.update_node_call: Invalid input - path must not contain empty parts or '/' and must have at least 4 segments")
                            raise ValueError("Invalid path format: must not contain empty parts or '/' and must have at least 4 segments")
                
                        # 检查 level_variant
                       
                        if not isinstance(level, int):
                            await self.wrapper.node.last_error_desc.write_value("CustomDAManage.update_node_call: Invalid input - level must be an integer")
                            raise ValueError("Level must be an integer")
                        if 0 < level < 2:
                            await self.wrapper.node.last_error_desc.write_value("CustomDAManage.update_node_call: Invalid input - level must be >= 2 or =0 ")
                        ...
                        await self.wrapper.node.last_error_code.write_value(ua.StatusCodes.BadInvalidArgument)
                        raise ua.UaStatusCodeError(ua.StatusCodes.BadInvalidArgument)   
                            ...
                            if self.wrapper.node.last_error_desc is not None: 
                                await self.wrapper.node.last_error_desc.write_value(f"CustomDAManage.update_node_call:Failed to call update_node ,Error Occured: {str(e)}")
                            raise           
        async def poll_item(self, parent, item: str) -> list:
            """OPC UA 方法：对指定 item 执行 async_poll 操作"""
            userrole = await self.wrapper.security._get_current_userrole()
            if not self.wrapper.user_manager.check_method_permission(50, userrole):  # 与 update_item 相同的权限级别
                logging.warning(f"CustomDAManage.poll_item: Unauthorized attempt to poll item {item}")
                await self.wrapper.node.last_error_code.write_value(ua.StatusCodes.BadUserAccessDenied)
                await self.wrapper.node.last_error_desc.write_value(f"CustomDAManage.poll_item: Unauthorized attempt to poll item {item}")
                raise ua.UaStatusCodeError(ua.StatusCodes.BadUserAccessDenied)
            ...
            try:
                ...
                await self.wrapper.node.last_error_desc.write_value(f"CustomDAManage.poll_item: Failed to poll item {item}, Error Occurred: {str(e)}")
        ...
        async def write_items(self,parent, values_variant, items_variant):
                userrole = await self.wrapper.security._get_current_userrole()
                if not self.wrapper.user_manager.check_method_permission(13, userrole):
                    logging.warning(f"CustomDAManage.write_items:Unauthorized attempt to call write_items ")
                    await self.wrapper.node.last_error_code.write_value(ua.StatusCodes.BadUserAccessDenied)
                    await self.wrapper.node.last_error_desc.write_value(f"CustomDAManage.write_items:Unauthorized attempt to call write_items ")
                ...
                try:
                    ...
                    for item, value, success in zip(items, values, results):
                        ...
                        if item not in self.wrapper.node.nodes:
                            ...
                            node = await self.wrapper.node.da_folder.add_variable(self.wrapper.node.idx, ua_name, value, varianttype=variant_type)
                            self.wrapper.node.nodes[item] = node
                        else:
                            node = self.wrapper.node.nodes[item]
        ...
        async def add_nodes_from_json(self, parent, json_data_variant) -> list:
                    ...
                    if not self.wrapper.user_manager.check_method_permission(50, userrole):
                        ...
                        await self.wrapper.node.last_error_code.write_value(ua.StatusCodes.BadUserAccessDenied)
                        await self.wrapper.node.last_error_desc.write_value("Unauthorized attempt to add nodes from JSON")
                        raise ua.UaStatusCodeError(ua.StatusCodes.BadUserAccessDenied)
        ...
        async def export_nodes_to_json(self, parent) -> list:
            ...
            if not self.wrapper.user_manager.check_method_permission(50, userrole):
                ...
                await self.wrapper.node.last_error_code.write_value(ua.StatusCodes.BadUserAccessDenied)
                await self.wrapper.node.last_error_desc.write_value("Unauthorized attempt to export nodes")
                raise ua.UaStatusCodeError(ua.StatusCodes.BadUserAccessDenied)

            try:
                # 构建节点结构的 JSON 表示
                node_structure = await self.wrapper.node._build_node_structure(self.wrapper.node.da_folder)
                ...
                await self.wrapper.node.last_error_desc.write_value(f"Error exporting nodes: {str(e)}")
                raise
...
class CustomHistoryManager:
    def __init__(self,wrapper,server_name: str = "DeltaV.OPCHDAsvr"):
        ...
        self.opchda = _OPCHDA_(server_name)
        self.wrapper = wrapper
    ...
    async def init(self):
        ...
        self.opchda_executor =self.wrapper.executor_opchda
    ...
    async def setup_event_type(self):
            ...
            try:
                ...
                # 定义 DeltaVEventType
                self.wrapper.node.event_type = await self.wrapper.server.nodes.base_event_type.add_object_type(self.wrapper.node.idx, "DeltaVEventType")
                event_properties = [
                ...
                for name, value in event_properties:
                    prop = await self.wrapper.node.event_type.add_property(self.wrapper.node.idx, name, value)
                    await prop.set_modelling_rule(True)
                logging.debug(f"CustomHistoryManager.setup_event_type: Created DeltaVEventType: {self.wrapper.node.event_type}")
                ...
                try:
                    condition_type_node = await self.wrapper.server.nodes.base_event_type.get_child(["0:ConditionType","0:AcknowledgeableConditionType","0:AlarmConditionType"])
                    ...
                    condition_type_node = await self.wrapper._find_type_node(self.server.nodes.base_event_type, "AlarmConditionType", namespace_index=0)
                    if not condition_type_node:
                        ...
                        # 回退到 ConditionType
                        condition_type_node = await self.wrapper._find_type_node(self.server.nodes.base_event_type, "ConditionType", namespace_index=0)
                        if not condition_type_node:
                            ...
                            condition_type_node = self.wrapper.server.nodes.base_event_type
                ...
                if node_class != ua.NodeClass.ObjectType:
                    logging.warning(f"CustomHistoryManager.setup_event_type: Node {condition_type_node} is not an ObjectType. Using BaseEventType as fallback.")
                    condition_type_node = self.wrapper.server.nodes.base_event_type
                self.wrapper.node.alarm_type = await condition_type_node.add_object_type(self.wrapper.node.idx, "DeltaVAlarmType")
                alarm_properties = [
                    ("EventTime", ua.Variant(datetime.datetime.now(), ua.VariantType.DateTime)),
                    ("Event_Type", ua.Variant("", ua.VariantType.String)),
                    ("Category", ua.Variant("", ua.VariantType.String)),
                ...
                for name, value in alarm_properties:
                    prop = await self.wrapper.node.alarm_type.add_property(self.wrapper.node.idx, name, value)
                    await prop.set_modelling_rule(True)
                logging.debug(f"CustomHistoryManager.setup_event_type: Created DeltaVAlarmType: {self.wrapper.node.alarm_type}")
    ...
    async def test_alarm(self):
                ...
                condition = self.wrapper.server.get_node(ua.NodeId(2830))
                
                con_gen = await self.wrapper.server.get_event_generator(condition, self.wrapper.node.events_node)  
                logging.debug(f"test_alarm:add get_event_generator: {con_gen}")
            

               
                alarm = self.wrapper.server.get_node(ua.NodeId(10637)) #10637
                alarm_gen = await self.wrapper.server.get_event_generator(alarm, self.wrapper.node.events_node)
    ...
    def read_opchda(self,item_id, start_time, end_time, num_values, return_bounds=3):
                ...
                try:
                    if self.opchda.connect():
                        results =  self.opchda.ReadRaw(item_id,start_time, end_time, num_values, return_bounds)
                        return results
                except:
                       logging.error(f"CustomHistoryManager:Failed to connect OPCHDA: {str(e)}")    
                finally:
                     self.opchda.disconnect()
    ...
    def sync_read_opchda(self,item_ids, start_time, end_time, max_values):
                    ...
                    try:
                        if self.opchda.connect():
                            results =  self.opchda.SyncReadRaw(item_ids, start_time, end_time, max_values)
                            return results
                    except:
                       logging.error(f"CustomHistoryManager:Failed to connect OPCHDA: {str(e)}")   
                    finally:
    ...
    async def historize_data(self, node_id: ua.NodeId, period: datetime.timedelta, count: Optional[int] = None):
        async with self._lock:
            logging.debug(f"CustomHistoryManager: Marked node {node_id} for data historization with period={period}, count={count}")
    ...
    async def read_data_history(self, node_id: ua.NodeId, params: ua.HistoryReadParameters):
        ...
        try:
            ...
            results = await asyncio.get_running_loop().run_in_executor(
                self.opchda_executor, self.read_opchda(item_id, start_time, end_time, num_values)
            )
            data_values = []
            for value, quality, timestamp in results.get(item_id, []):
                ...
                status = ua.StatusCode(ua.StatusCodes.Good) if quality > 127 else ua.StatusCode(ua.StatusCodes.Bad)
                data_value = ua.DataValue(
                    Value=ua.Variant(value, ua_utils.guess_datatype(type(value))),
                    StatusCode_=status,
    ...
    async def sync_read_raw_history(self,parent, json_variant):
            userrole = await self.wrapper.security._get_current_userrole()
            if not self.wrapper.user_manager.check_method_permission(50, userrole):
                    logging.warning(f"CustomHistoryManager.query_event_history: Unauthorized attempt to query event history")
                    await self.wrapper.node.last_error_code.write_value(ua.StatusCodes.BadUserAccessDenied)
                    await self.wrapper.node.last_error_desc.write_value("Unauthorized attempt to query event history")
                    raise ua.UaStatusCodeError(ua.StatusCodes.BadUserAccessDenied)
            ...
            try:
                ...
                results = await asyncio.get_running_loop().run_in_executor(
                    self.opchda_executor, self.sync_read_opchda(item_ids, start_time, end_time, max_values)
                )
                ...
                for item_id in item_ids:
                    ...
                    for value, quality, timestamp in results.get(item_id, []):
                        ...
                        quality_str = "Good" if quality > 127 else "Bad"
                        data_points.append({
                            "value": value,
                            "quality": quality_str,
    ...
    async def update_events(self, events):
                ...
                for node_id in list(self.wrapper.node.event_nodes.keys()):
                    try:
                        condition_node = self.wrapper.node.event_nodes[node_id]
                        if hasattr(condition_node, 'nodeid'):
                            await self.wrapper.server.delete_nodes([condition_node], recursive=True)
                            del self.wrapper.node.event_nodes[node_id]
                ...
                for event in events:
                    ...
                    if event_data is not None:
                            await self.save_event( self.wrapper.node.events_node.nodeid,event_data)
                    ...
                    try:
                        if event['Event_Type'] == "ALARM":
                            ...
                            condition_node = await self.wrapper.node.events_node.add_object(self.wrapper.node.idx, f"Condition_{event_id}", objecttype=self.wrapper.node.alarm_type)
                            
                            self.wrapper.node.event_nodes[event_id] = condition_node
                            condition_id = ua.NodeId(f"Condition_{event_id}", self.wrapper.node.idx)
                            ...
                            for name, value in  event_data.items():
                                ...
                                if prop_node:
                                        ...
                                        prop_node = await condition_node.add_property(self.wrapper.node.idx, name, value)
                            now = datetime.datetime.now(datetime.UTC)
                            standard_properties = {
                              
                                "ConditionId": condition_id,  
                           
                                "SourceNode": ua.Variant(self.wrapper.node.events_node.nodeid, ua.VariantType.NodeId),
                                
                                "EventType": ua.Variant(self.wrapper.node.alarm_type.nodeid, ua.VariantType.NodeId),
                               
                                "InputNode" : ua.Variant(self.wrapper.node.events_node.nodeid, ua.VariantType.NodeId),
                            ...
                            event_generator = await self.wrapper.server.get_event_generator(self.wrapper.node.alarm_type, self.wrapper.node.events_node)
                            ...
                            standard_properties = {                                                                   
                                "SourceNode" :  ua.Variant(self.wrapper.node.events_node.nodeid, ua.VariantType.NodeId),           
                                "EventType" : ua.Variant(self.wrapper.node.event_type.nodeid, ua.VariantType.NodeId), 
                             
                                "Message" : ua.LocalizedText(f"✅ Event {event['Module']} {event['Attribute']}")                             
                              
                            }
                         
                            event_generator = await self.wrapper.server.get_event_generator(self.wrapper.node.event_type, self.wrapper.node.events_node)
                            event_data.update(standard_properties)
    ...
    async def periodic_event_update(self,filters,server_name="localhost"):
    ...
    async def _create_events_nodes(self,events):
        ...
        for node_id in list(self.wrapper.node.event_nodes.keys()):
            try:
                child_node = await self.wrapper.node.events_node.get_child(f"{self.wrapper.node.idx}:{node_id}")
                await self.wrapper.node.events_node.delete_nodes([child_node], recursive=True)
            except Exception as e:
                logging.warning(f"CustomHistoryManager._create_events_nodes:Failed to delete node {node_id}: {str(e)}")
        self.wrapper.node.event_nodes.clear()

        for event in events:

            event_id = f"{event['Module']}_{event['Ord']}"
            try:
                event_node = await self.wrapper.node.events_node.add_object(self.wrapper.node.idx, event_id)
                self.wrapper.node.event_nodes[event_id] = {
                    "EventTime": await event_node.add_variable(self.wrapper.node.idx, "EventTime", event["EventTime"]),
                    "EventType": await event_node.add_variable(self.wrapper.node.idx, "EventType", event["Event_Type"]),
                    "Category": await event_node.add_variable(self.wrapper.node.idx, "Category", event["Category"]),
                    "Area": await event_node.add_variable(self.wrapper.node.idx, "Area", event["Area"]),
                    "Node": await event_node.add_variable(self.wrapper.node.idx, "Node", event["Node"]),
                    "Module": await event_node.add_variable(self.wrapper.node.idx, "Module", event["Module"]),
                    "ModuleDescription": await event_node.add_variable(self.wrapper.node.idx, "ModuleDescription", event["ModuleDescription"]),
                    "Attribute": await event_node.add_variable(self.wrapper.node.idx, "Attribute", event["Attribute"]),
                    "State": await event_node.add_variable(self.wrapper.node.idx, "State", event["State"]),
                    "Level": await event_node.add_variable(self.wrapper.node.idx, "Level", event["Level"]),
                    "Parameter": await event_node.add_variable(self.wrapper.node.idx, "Parameter", event["Parameter"]),
                    "Description": await event_node.add_variable(self.wrapper.node.idx, "Description", event["Description"])
                }
                for key, node in self.wrapper.node.event_nodes[event_id].items():
                ...
                try:
                    ...
                    if filters_str:
                        try:
                            ...
                            await self.wrapper.node.last_error_desc.write_value(f"Invalid JSON filters: {str(e)}")
                            return [ua.Variant("", ua.VariantType.String)]
                        except ValueError as e:
                    ...
                    await self.wrapper.node.last_error_desc.write_value(f"Error querying events: {str(e)}")
                    return [ua.Variant("", ua.VariantType.String)]
...
class _OPCDAWrapper_:
    ...
    def __init__(self,  name:str= 'OPC.DELTAV.1',nodename:str="PROPLUS", endpoint: str = 'opc.tcp://0.0.0.0:4840',config: Dict = None):
        """init the server"""
    ...
    async def setup_opc_ua_server(self):
        ...
        if not os.path.exists(self.security.initial_cert_path) or not os.path.exists(self.security.initial_key_path):
            logging.info("_OPCDAWrapper_.setup_opc_ua_server:init Certificate or key not found, generating new ones...")
            await self.security.generate_self_signed_cert(self.security.initial_cert_path, self.security.initial_key_path)
        ...
        cert_path = self.security.cert_path if os.path.exists(self.security.cert_path) else self.security.initial_cert_path
        key_path = self.security.key_path if os.path.exists(self.security.key_path) else self.security.initial_key_path
        ...
        for policy in self.security.security_policies:
                    if policy != SecurityPolicy():
                        policy.ClientCertificateDir = self.security.trustedcert_dir
        ...
        method_nodes = {
            ...
            "QueryEventHistory": await self.node.events_node.add_method(
             ...
             "SyncReadRawHistory": await self.node.historian_node.add_method(
                    self.node.idx,
                    "SyncReadRawHistory",
                    self.history_manager.sync_read_raw_history,
                    [ua.VariantType.String],  # Input: JSON string for filters
                    [ua.VariantType.String]   # Output: JSON string of events
                )
    ...
    async def start(self):
            ...
            async with self.server:
                try:
                    ...
                    event_update_task = asyncio.create_task( self.history_manager.periodic_event_update(filters))
...
async def main(max_time: Optional[float] = None, max_count: Optional[int] = None, manual_stop: bool = False):
    ...
    try:
        ...
        # 启动服务
        wrapper.da_manager.append_items(items)
        wrapper.da_manager.set_max_updates(max_count)
        subscription_task = asyncio.create_task(wrapper.start())
        ...
        await asyncio.sleep(10)
        # wrapper.da_manager.remove_items(items2)
