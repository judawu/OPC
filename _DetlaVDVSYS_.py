...
class _OPCDA_:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            logging.debug(f"_OPCDA_: dir(self.opc): {dir(self.opc)}")
            logging.debug(f"_OPCDA_: self.opc Servers: {self.opc.GetOPCServers()}")
           
            #self.opc = win32com.client.Dispatch("DeltaV.DVSYSsvr.1")
            self.opc.Connect(self.server_name)
            logging.debug(f"_OPCDA_: opc server: {self.opc.VendorInfo}")
           
            #self.browser = self.opc.CreateBrowser()
            
            self.groups = self.opc.OPCGroups  # 为read方法准备
            print(dir(self.groups))
            print(self.groups.default_interface)
            print(self.groups.coclass_interfaces)
            print(self.groups.coclass_sources)
            print(self.groups.default_source)
    ...
    def subscribe(self, item_paths: List[str], group_name: str = "SubscribeGroup", update_rate: int = 1000, callback: Optional[Callable[[List[str], List[Tuple[any, int, str]]], None]] = None) -> None:
        ...
        try:
            ...
            opc_items = []
            for i, item_path in enumerate(item_paths, 1):  # ClientHandle 从 1 开始
                opc_item = group.OPCItems.AddItem(item_path, i)
                opc_items.append((item_path, i))  # 存储 (path, client_handle)
